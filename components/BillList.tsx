      ...
      if (!response.ok) {
        ...
        if (fetchedBills.length > 0) {
          setUniquePosts(
            Array.from(
              new Set(
                fetchedBills.map((bill: IBill) => bill.post).filter(Boolean),
              ),
            ),
          );
        }
    ...
    <div className="w-full px-4 py-6 md:py-10 md:px-6">
      ...
      {adminMode && storageUsed && (
        <Popover width={300} position="bottom-end" withArrow shadow="md">
          <Popover.Target>
            <span className="font-mono text-sm md:text-base pb-1 cursor-pointer underline decoration-dotted underline-offset-4 hover:opacity-80 transition-opacity">
          ...
          <Popover.Dropdown>
            ...
            <Text size="xs" color="dimmed">
              ...
              <br />
              <br />
              ⚠️ <strong>Warning:</strong> If you exceed 100%, Supabase will
              lock the project entirely and require payment to restore access.
              Please clean up old files if this turns red.
            </Text>
          </Popover.Dropdown>
        </Popover>
      )}
