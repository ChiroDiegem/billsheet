...
export default function BillListItem({
  ...
  async function handleBookedChange(e: any) {
    ...
    try {
      ...
      notifications.show({
        title: "Error",
        message: "Failed to update booked status",
      });
    }
  }
          ...
          <Box py="md">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <Text size="sm">
                  {
                    PAYMENT_METHODS.find((x) => {
                      return x.value == bill.payment_method;
                    })?.label
                  }
                </Text>
              </div>
              ...
              {adminMode && (
                <>
                  <button
                    ...
                    className={`inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-white ${isSendingEmail ? "bg-gray-400 cursor-not-allowed" : "bg-black hover:bg-gray-800"}`}
                  >
