  ...
  const updateBill = async (event: FormEvent) => {
    ...
    try {
      ...
      const billData = {
        ...
        amount: Math.round(
          parseFloat(String(amountValue).replace(",", ".")) * 100,
        ),
        payment_method: paymentMethod || PAYMENT_METHODS[0].value,
    ...
    <div className="flex justify-center align-center border-2 border-primary-color rounded-lg p-4 sm:p-10">
      ...
      >
        ...
        <TextInput
          ...
          ref={activityRef}
        />
        <TextInput label="Description" defaultValue={bill.desc} ref={descRef} />
        <NumberInput
          label="Amount"
          required
